...
util.setBotName("HabBeetrootBot");
...
const depositCrop = 1970;
const depositSeed = 1965;
...
function returnMode() {
  ...
  // Go drop items in the deposit that's furthest from the start
  if (playerX < Math.min(depositSeed, depositCrop)) {
    // Go to the deposit site, in a straight line
    util.eatCheck(foodSlot);
    util.player.lookAt(xMax + 0.5, util.player.getY(), zMax + 0.5);
    util.moveTo(
      ["key.sprint", "key.jump"],
      Math.min(depositSeed, depositCrop) + 0.5,
      zMax + 0.5,
    );
  ...
  if (playerX < Math.max(depositSeed, depositCrop)) {
    ...
    util.moveTo(
      ["key.sprint", "key.jump"],
      Math.max(depositSeed, depositCrop) + 0.5,
      zMax + 0.5,
    );
    if (util.isTerminated) {
      return;
...
function checkDeposit(playerX, playerZ) {
  ...
  if (playerX == depositCrop) {
    itemToDrop = crop;
  } else if (playerX == depositSeed) {
    itemToDrop = getSeeds();
  } else {
    return;
  }
  ...
  // Drop items
  if (util.hasAnyItem([itemToDrop], false, [9, 44])) {
    // Walk to center of block
    util.moveTo([], playerX + 0.5, playerZ + 0.5, 0.2);
    // Look at collection system slightly angled up
    // as to not accidentally get stuck looking at a sign menu
    util.player.lookAt(0, -10);
    Client.waitTick();
    util.dropAll([itemToDrop]);
  }
}
